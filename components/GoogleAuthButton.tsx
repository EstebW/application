'use client'

import { useState } from 'react'
import { motion } from 'framer-motion'
import { signInWithGoogle, formatAuthError } from '@/lib/auth'

function GoogleGlyph({ size = 18 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden>
      <path
        fill="#FFC107"
        d="M43.611 20.083H42V20H24v8h11.303C33.654 32.657 29.209 36 24 36c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"
      />
      <path
        fill="#FF3D00"
        d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.188 0-9.62-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 01-4.087 5.571l.003-.002 6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z"
      />
    </svg>
  )
}

type Props = {
  /** Chemin après callback, ex. `/dashboard` ou `/?oauth=funnel` */
  nextPath?: string
  /** Appelé juste avant la redirection Google (sauvegarder le funnel, etc.) */
  onBeforeRedirect?: () => void
  label?: string
  className?: string
  onError?: (message: string) => void
}

export default function GoogleAuthButton({
  nextPath = '/dashboard',
  onBeforeRedirect,
  label = 'Continuer avec Google',
  className = '',
  onError,
}: Props) {
  const [loading, setLoading] = useState(false)

  async function handleClick() {
    if (loading) return
    setLoading(true)
    try {
      onBeforeRedirect?.()
      await signInWithGoogle(nextPath)
      // Redirection Google — pas de reset loading
    } catch (err) {
      const message = formatAuthError(err instanceof Error ? err.message : 'Erreur Google')
      onError?.(message)
      setLoading(false)
    }
  }

  return (
    <motion.button
      type="button"
      onClick={() => { void handleClick() }}
      disabled={loading}
      whileHover={loading ? {} : { scale: 1.01 }}
      whileTap={loading ? {} : { scale: 0.99 }}
      className={`w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-semibold disabled:opacity-70 ${className}`}
      style={{
        background: 'rgba(255,255,255,0.04)',
        border: '1px solid rgba(255,255,255,0.12)',
        color: '#E8E8E8',
      }}
    >
      {loading ? (
        <motion.div
          className="w-4 h-4 rounded-full border-2 border-white/25 border-t-white"
          animate={{ rotate: 360 }}
          transition={{ duration: 0.8, repeat: Infinity, ease: 'linear' }}
        />
      ) : (
        <GoogleGlyph />
      )}
      {loading ? 'Redirection Google…' : label}
    </motion.button>
  )
}
